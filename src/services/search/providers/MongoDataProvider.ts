...
export async function getPerson1(id: string) {
...
export async function getFather1(id: string) {
  console.log("GetFather")
  const client = await initClient();
  const db = client.db(mongoDbDatabase);
  let collection = db.collection(relationCollection);
  console.log(id)
  let getRelationQuery = { "Person2": ObjectId(id), "Type": "Parent" }
  let rels = await collection.find(getRelationQuery).toArray();

  console.log(JSON.stringify(rels))
  let collection2 = db.collection(memberCollection);
  let res2 = await collection2.find({ _id: { $in: rels } }).toArray();
  return res2[0];
...
async function getRelationFromCollection(client: any, id: any) {
  ...
  let res2 = await collection2.find({ _id: { $in: items } }, {
    fields: {
      _id: 1,
      FirstName: 1,
      LastName: 1,
